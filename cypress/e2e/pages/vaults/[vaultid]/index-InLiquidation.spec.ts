context("/vaults/[vaultid] - In Liquidation on desktop", () => {
  ...
  describe("should Vault Details", () => {
    ...
    it("should have Vault Details and 4 cells in each row", () => {
      cy.findAllByTestId("OverflowTable.Row").within(() => {
        ...
        cy.findByTestId("DesktopVaultDetailsRow.TotalLoanValue")
          .should("be.visible")
          .contains(/^\$\d{1,3}(,\d{3})*(\.\d+)$/);
        cy.findByTestId("DesktopVaultDetailsRow.TotalCollateralValue")
          .should("be.visible")
          .contains(/^\$\d{1,3}(,\d{3})*(\.\d+)$/);
        cy.findByTestId("DesktopVaultDetailsRow.APR")
          .should("be.visible")
          .contains(/^\d{1,3}(,\d{3})*(\.\d+)%$/);
      });
    });

    it("should not have health bar", () => {
      cy.findByTestId("VaultHealthBar").should("not.exist");
    });
  });

  describe("should have Auctions Details", () => {
    it("should Vault Details Heading", () => {
    ...
    it("should have OverflowTable header information", () => {
      cy.findByTestId("AuctionTable.TimeLeft")
        .should("be.visible")
        .should("have.text", "Time Left");
      cy.findByTestId("AuctionTable.TimeLeft").within(() => {
        cy.findByTestId("InfoHoverPopover").should("be.visible");
      });
      cy.findByTestId("AuctionTable.LoanToken")
        .should("be.visible")
        .should("have.text", "Loan Token");
      cy.findByTestId("AuctionTable.CollateralForAuction")
        .should("be.visible")
    ...
    it("should have Auction Details and 5 cells in each row", () => {
      cy.findAllByTestId("VaultAuctionsDesktop").within(() => {
        cy.findAllByTestId("OverflowTable.Row").within(() => {
          ...
          cy.findByTestId("AuctionsTableRow.AuctionTimeLeft")
            .should("be.visible")
            .contains(/^.* left$/);
...
context("/vaults/[vaultid] - In Liquidation on mobile", () => {
  ...
  describe("should Vault Details", () => {
    ...
    it("should have Vault Details", () => {
      cy.findByTestId("VaultDetailList.OwnerAddress").within(() => {
        cy.findByTestId("VaultDetailsListItem.Title")
          .should("be.visible")
          .should("have.text", "Owner's Address");
  ...
  describe("should have Auction Details", () => {
