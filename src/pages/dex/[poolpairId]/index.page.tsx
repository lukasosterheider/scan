  ...
  useEffect(() => {
    ...
    if (next === undefined) {
      const interval = setInterval(() => {
        ...
        void api.poolpairs
          .listPoolSwapsVerbose(props.poolpair.id, 10)
          .then((data) => {
            setSwapItems(data);
          });
      }, 10000);
    ...
    <>
      ...
      <Container className="pt-4 pb-20">
        ...
        <div className="flex flex-wrap flex-row lg:space-x-4">
          ...
          {["DUSD-DFI", "dUSDC-DUSD", "dUSDT-DUSD", "dEUROC-DUSD"].includes(
            ...
            <PoolPairInfo
              ...
              lhsComponent={() => (
                <span className="flex items-center dark:text-gray-400 mr-2">
        ...
        <div className="flex justify-end mt-8">
          <CursorPagination
            pages={props.swaps.pages}
            path={`/dex/${props.poolpair.id}`}
          />
        </div>
      </Container>
    </>
...
function getPoolPairsByParam(
  ...
  return poolpairs
    .filter((pair) => {
      if (tokenB !== undefined) {
        return (
          pair.tokenA.displaySymbol.toLowerCase() === tokenA.toLowerCase() &&
          pair.tokenB.displaySymbol.toLowerCase() === tokenB.toLowerCase()
        );
      } else {
        return (
          pair.tokenA.displaySymbol.toLowerCase() === param.toLowerCase() ||
          pair.tokenA.symbol.toLowerCase() === param.toLowerCase()
        );
      }
    })
    .pop();
